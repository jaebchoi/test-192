...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-192-infrastructure \
     --values test-192-infrastructure/values.yaml \
     --values test-192-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/jaebchoi/test-192 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "test-192 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-192 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace test-192 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/jaebchoi/test-192 \
      --path test-192-deploy/src/main/resources \
      --revision $branch \
